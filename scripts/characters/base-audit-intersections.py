...
Run with Blender: --background --python-exit-code 1 --python scripts/characters/base-audit-intersections.py -- --character Base_Male
"""
...
import argparse, sys
parser=argparse.ArgumentParser();parser.add_argument('--character',required=True,choices=['Base_Male','Base_Female'])
NAME=parser.parse_args(sys.argv[sys.argv.index('--')+1:]).character
FOLDER=ROOT/'resources'/f'{NAME.lower().replace(chr(95),chr(45))}-progression'
...
base,faces=load(FOLDER/'work/base.glb')
_,weld=np.unique(base,axis=0,return_inverse=True)
welded= weld[faces]
results=[];base_pairs=None
for stage,path in [('Base',FOLDER/'work/base.glb')]+[(s,FOLDER/f'{NAME}_{s}.glb') for s in ['Starter','Developing','Strong','Elite','Legendary']]:
    ...
    results.append(row);print(json.dumps({k:v for k,v in row.items() if k not in ['intersectionPairs','examples']}),flush=True)
