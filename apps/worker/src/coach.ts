import {
  ...
  MAX_COACH_QUESTION_CHARS, type CoachMessage, type CoachQuestion, type CoachReply, type CoachSource,
} from '@vyra/core';
...
function knowledgeReply(sources: CoachSource[], notice?: string): CoachReply {
  ...
  return { answer, mode: 'knowledge', sources, messageId: crypto.randomUUID() };
}
...
export async function handleCoachChat(request: Request, env: CoachEnv, playerId: string): Promise<CoachReply> {
  ...
  if (!sources.length || (!env.HF_TOKEN?.trim() && (String(env.AI_ENABLED) !== 'true' || !env.AI))) return knowledgeReply(sources);
  try {
    ...
    if (!quota.allowed) {
      ...
      return knowledgeReply(sources, `AI replies are temporarily limited. Try again in about ${minutes} minute${minutes === 1 ? '' : 's'}. Here are the relevant guide passages:`);
    }
  } catch { return knowledgeReply(sources, 'AI replies are temporarily unavailable. Here are the relevant guide passages:'); }
  const answer = await generateCoachAnswer(env, input, sources);
  return answer ? { answer, mode: 'generated', sources, messageId: crypto.randomUUID() }
    : knowledgeReply(sources, 'AI replies are temporarily unavailable. Here are the relevant guide passages:');
}
