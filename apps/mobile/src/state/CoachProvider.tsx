...
function isExerciseSuggestion(value: unknown): boolean {
  if (!value || typeof value !== 'object') return false;
  const ex = value as Record<string, unknown>;
  return typeof ex.slug === 'string' && typeof ex.name === 'string' && typeof ex.subtitle === 'string'
    && Array.isArray(ex.primaryMuscles) && ex.primaryMuscles.every(m => typeof m === 'string')
    && Array.isArray(ex.secondaryMuscles) && ex.secondaryMuscles.every(m => typeof m === 'string')
    && Array.isArray(ex.equipment) && ex.equipment.every(item => typeof item === 'string')
    && (ex.image === null || typeof ex.image === 'string');
}
...
function isCoachReply(value: unknown): value is CoachReply {
  ...
  return typeof reply.answer === 'string' && !!reply.answer.trim()
    ...
    // Exercise cards are optional; when present every entry must be a well-formed suggestion.
    && (reply.exercises === undefined || (Array.isArray(reply.exercises) && reply.exercises.every(isExerciseSuggestion)));
}
