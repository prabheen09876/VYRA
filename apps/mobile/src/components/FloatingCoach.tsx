...
import { MAX_COACH_QUESTION_CHARS, MUSCLE_GROUP_LABELS, type CoachSource, type ExerciseSuggestion } from '@vyra/core';
...
export default function FloatingCoach() {
  const { booting, profile, session } = useApp();
  ...
  // Hand off to the full library, deep-linked to this exercise's how-to.
  const openExercise = (slug: string) => { coach.closeCoach(); router.push({ pathname: '/exercises', params: { slug } }); };
              ...
              {coach.exchanges.map(exchange => <View key={exchange.key} style={styles.exchange}>
                ...
                {exchange.status === 'pending' ? <View accessibilityLiveRegion="polite" style={styles.pending}><ActivityIndicator size="small" color={colors.brand} /><Text style={styles.copy}>Finding your guidance…</Text></View>
                  : exchange.status === 'failed' ? <View accessibilityLiveRegion="polite" style={styles.failure}>
                    ...
                    {!!exchange.reply.exercises?.length && <View style={styles.exerciseCards}>{exchange.reply.exercises.map(exercise =>
                      <ExerciseCard key={exercise.slug} exercise={exercise} apiUrl={session.apiUrl} onOpen={() => openExercise(exercise.slug)} />)}</View>}
...
function ExerciseCard({ exercise, apiUrl, onOpen }: { exercise: ExerciseSuggestion; apiUrl: string; onOpen: () => void }) {
  const [failed, setFailed] = useState(false);
  const muscles = exercise.primaryMuscles.map(group => MUSCLE_GROUP_LABELS[group]).join(' · ');
  return <Pressable accessibilityRole="button" accessibilityLabel={`${exercise.name}. ${exercise.subtitle}. Open in exercise library.`}
    onPress={onOpen} style={({ pressed }) => [styles.exerciseCard, pressed && styles.pressed]}>
    {exercise.image && !failed
      ? <Image accessible={false} source={{ uri: `${apiUrl}${exercise.image}` }} onError={() => setFailed(true)} resizeMode="contain" style={styles.exerciseThumb} />
      : <View style={[styles.exerciseThumb, styles.exerciseThumbFallback]}><Text style={styles.exerciseThumbMark}>{exercise.name.slice(0, 1).toUpperCase()}</Text></View>}
    <View style={styles.exerciseInfo}>
      <Text style={styles.exerciseName} numberOfLines={1}>{exercise.name}</Text>
      <Text style={styles.exerciseMeta} numberOfLines={1}>{muscles || exercise.subtitle}</Text>
    </View>
...
const styles = StyleSheet.create({
  ...
  exerciseCards: { gap: 8, marginTop: 2 },
  exerciseCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 10, borderRadius: 12, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.glass },
  exerciseThumb: { width: 46, height: 46, borderRadius: 8, backgroundColor: alpha(colors.brand, 0.06) },
  exerciseThumbFallback: { alignItems: 'center', justifyContent: 'center', backgroundColor: colors.glassStrong },
  exerciseThumbMark: { fontFamily: fonts.display, fontSize: 20, fontWeight: displayWeight.heavy, color: colors.faint },
  exerciseInfo: { flex: 1, minWidth: 0, gap: 2 }, exerciseName: { fontFamily: fonts.body, fontSize: 13, fontWeight: '700', color: colors.text }, exerciseMeta: { fontFamily: fonts.body, fontSize: 11, lineHeight: 16, color: colors.muted, textTransform: 'uppercase', letterSpacing: 0.6 },
  exerciseChevron: { fontFamily: fonts.body, fontSize: 22, color: colors.brand },
