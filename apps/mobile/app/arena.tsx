...
export default function ArenaScreen() {
  ...
  return <Screen noNav back={() => router.replace('/')}>
    ...
    {searching && <Notice tone="info" title="Searching for an opponent…" action={() => { cancelMatchmaking(); setPending(null); }} actionLabel="Cancel search">Hang tight — this jumps straight into a private match the moment someone else is found.</Notice>}
    <View style={styles.actions}>
      <Button onPress={() => begin()} loading={pending === 'create'} disabled={!!pending} style={{ flex: 1 }}>{!profile ? 'Create your player' : !calibrated ? 'Prepare my camera' : mode === 'solo' ? 'Start solo training' : 'Create private room'}</Button>
...
const styles = StyleSheet.create({
  ...
  symbol: { fontSize: 49, lineHeight: 58 }, radio: { width: 23, height: 23, borderRadius: 12, borderWidth: 2, borderColor: colors.lineStrong },
  modeTitle: { fontFamily: fonts.display, color: colors.text, fontSize: 29, fontWeight: '800', letterSpacing: -0.9 },
