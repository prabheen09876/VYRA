...
export default function TrainingScreen() {
  ...
  const visible = cameraOpen && focused && isLivePoseFresh(livePose.current) && !!tracking?.visible;
  ...
  return <Screen>
    ...
    <View style={[styles.setBar, wide && { flexDirection: 'row', alignItems: 'center' }]}>
      <View style={styles.metrics}>
        <Stat value={reps} label="valid reps" color={colors.brand} />
        <Stat value={timeLabel(totalTime)} label="set time" />
        <Stat value={reps ? `${Math.round(formTotal / reps)}%` : '—'} label="average form" />
      </View>
    ...
    <Text accessibilityLiveRegion="polite" style={styles.cue}>{summary ? 'Practice complete. Head to the Arena when you want a workout that earns XP.' : phase === 'paused' ? 'Set paused. Reopen the camera if needed, then resume when you are in position.' : phase === 'running' ? visible ? (tracking?.cue || 'Complete each movement and return to the starting position.') : 'Tracking lost. Move fully into frame to continue counting.' : 'Enable your camera and move into frame to start. Changing exercise starts a new set.'}</Text>
...
const styles = StyleSheet.create({
  ...
  cue: { fontFamily: fonts.body, color: colors.muted, fontSize: 14, lineHeight: 23, paddingVertical: 20 },
