...
The [training screen](../apps/mobile/app/train.tsx) offers squats and push-ups, camera setup, and a tracking overlay that is on by default and can be hidden. Valid reps, set time, average rep form, and Start/Pause/Resume/Finish controls sit directly below the training camera. Camera access starts through explicit user controls. Leaving the screen closes capture; returning requires camera setup again. The capture page also exposes Stop and Mirror controls. Typed camera-state messages notify the host when capture starts or stops; Stop, ended camera tracks, page hiding, and capture errors clear the local pose.

[CaptureSurface](../apps/mobile/src/components/CaptureSurface.tsx) displays shared [tracking feedback](../apps/mobile/src/components/TrackingFeedback.tsx) under the camera: **Live form**, **Movement**, **Tracking confidence**, and **Tracking rate** in FPS. Live form is the current posture estimate from the geometric movement rules. Tracking confidence comes from the detected landmarks, while FPS measures how often frames are processed; neither is a measured model-accuracy percentage. During warmup, the movement reads **Calibrating** and confidence/rate can appear before live form is available. Unavailable readings use a dash; a valid zero form score remains `0%`. Stop, reset, error, and stale-message states clear the live readings.

**Average rep form** averages scores attached to completed reps in the current set. Each rep currently retains the lowest score from its recognized top/bottom frames. This differs from the changing live-form reading and does not average every camera frame.
...
A focused harness exercised the actual CaptureSurface and TrackingFeedback components for warmup, valid zero scores, movement/confidence/FPS, tracking loss, stale messages, reset, error, stop, overlay preservation, and cleanup. Those checks passed. Fresh browser visual checks of the feedback and camera-adjacent controls remain unavailable because browser tooling lacks a Codex auth token; physical-camera accuracy was not tested.
